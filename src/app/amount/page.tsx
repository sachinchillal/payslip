...
import { Company, Table } from '@/interface/app';
import { getPayslipData } from '@/store/getters';
import { deleteTable, saveOrUpdateTable, saveTables } from '@/store/setters';
import { Layers2 } from 'lucide-react';
import React, { useState, useEffect } from 'react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm'

const sample = `> **Payslip - March 2025**

| Earnings             | Amount (₹) | Deductions          | Amount (₹) |
|----------------------|------------|---------------------|------------|
| Basic Salary         | 50,000.00  | Income Tax          | 5,000.00   |
| House Rent Allowance | 20,000.00  | Provident Fund      | 3,600.00   |
| Special Allowance    | 10,000.00  | Professional Tax    | 200.00     |
| Performance Bonus    | 5,000.00   | Health Insurance    | 500.00     |
| **Gross Earnings** | **85,000.00** | **Total Deductions** | **9,300.00** |
| **Net Payable** |            |                     | **75,700.00** |
| *(Gross Earnings - Total Deductions)* |            |                     |            |
`;
const sample2 = `> **Payslip - March 2026**
...
export default function CompanyDetailsEditor() {
  ...
  const handleSaveDetails = () => {
    const res = saveOrUpdateTable(companyDetails, companyLabel, companyId);
  ...
  const handleViewSample = () => {
    ...
    setCompanyLabel('Sample Table Details');
    showStatus("Sample Table details loaded.", "success");
  };
  const handleViewSample2 = () => {
    resetVariables();
    setCompanyDetails(sample2);
    setCompanyLabel('Sample Table Details 2');
    showStatus("Sample Table details loaded.", "success");
  };
  ...
  const handleDuplicateDetail = (c: Company) => {
    ...
    if (!res.error) {
      init();
      resetVariables();
      setEditingCompanyIndex(null); // Reset editing index after duplication
    }
    window.scrollTo({ top: 0, behavior: 'smooth' }); // Scroll to top for better UX
  }

  const handleRemoveDetail = (c: Company) => {
    const res = deleteTable(c.id);
    showStatus(res.message, res.error ? 'error' : 'success');
    if (!res.error) {
      init();
      resetVariables();
  ...
  const handleOnSelect = (t: Table) => {
    console.log(t)
    console.log(companies)
    t.isSelected = !t.isSelected;
    t.selectedAt = Date.now();
    setCompanies([...companies]); // Update state to trigger re-render
    setSelectedTables(companies.filter(t => t.isSelected).sort((a, b) => a.selectedAt - b.selectedAt));
    saveTables(companies);
  }
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 sm:p-6 lg:p-8 font-sans">
      <div className="tmax-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-blue-600 dark:text-blue-400">Table Details Editor</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-semibold mb-4 text-gray-700 dark:text-gray-300">
              {editingCompanyIndex !== null ? `Editing Table #${editingCompanyIndex + 1}` : 'Enter Table Details (Markdown)'}
            </h2>
            ...
            <textarea
              ...
              placeholder="Enter Table details here. Use Markdown for formatting..."
              rows={15}
        ...
        <div className="mt-12 bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg">
          <h2 className="text-xl sm:text-2xl font-semibold mb-6 text-gray-700 dark:text-gray-300">
            Stored Table Details
          </h2>
          {companies.length === 0 ? (
            <p className="text-gray-500 dark:text-gray-400">No Tables details saved yet.</p>
          ) : (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {companies.map((c, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400" onClick={() => handleOnSelect(c)}>{index + 1}</td>
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100 cursor-pointer" onClick={() => handleOnSelect(c)}>
                        <div className="max-w-xs sm:max-w-md md:max-w-lg truncate" title={c.label}>
                          {c.label || `Table Entry ${index + 1}`}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100 cursor-pointer" onClick={() => handleOnSelect(c)}>
                        <div className="max-w-xs sm:max-w-md md:max-w-lg truncate" title={c.value}>
                          {c.value.split('\n')[0].trim() || `Table Entry ${index + 1}`}
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                      {selectedTables.length === 0 ? "No tables selected." : `Selected ${selectedTables.length} table(s). Are in the order of selection.`}
                    </td>
                  </tr>
                  {
                    selectedTables.map((t, index) => (
                      <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{t.label || `Table Entry ${index + 1}`}</td>
                        <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                          <div className="max-w-xs sm:max-w-md md:max-w-lg truncate" title={t.value}>
                            {t.value.split('\n')[0].trim() || `Table Entry ${index + 1}`}
                          </div>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{new Date(t.createdAt).toLocaleString()}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{new Date(t.updatedAt).toLocaleString()}</td>
                      </tr>
                    ))
                  }
