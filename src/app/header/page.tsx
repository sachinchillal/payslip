...
import { deleteCompany, saveOrUpdateCompanyDetail, selectCompany } from '@/store/setters';
import React, { useState, useEffect } from 'react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm'

const sampleCompanyDetails1 = `
...
- **Phone:** (555) 123-9876
`;
const sampleCompanyDetails = `| Earnings             | Amount (₹) | Deductions          | Amount (₹) |
|----------------------|------------|---------------------|------------|
...
`;
const sampleCompanyDetails2 = `| Earnings             | Amount (₹) | Deductions          | Amount (₹) |
|----------------------|------------|---------------------|------------|
| Basic Salary         | 50,000.00  | Income Tax          | 5,000.00   |
| House Rent Allowance | 20,000.00  | Provident Fund      | 3,600.00   |
| Special Allowance    | 10,000.00  | Professional Tax    | 200.00     |
...
export default function CompanyDetailsEditor() {
  ...
  const [selectedCompanyId, setSelectedCompanyId] = useState<number | null>(null); // For company ID if needed
  ...
  const init = () => {
    ...
    setSelectedCompanyId(p.selectedCompanyId);
  }
  ...
  const handleViewSample = () => {
    resetVariables();
    setCompanyDetails(sampleCompanyDetails);
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 sm:p-6 lg:p-8 font-sans">
      <div className="tmax-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-blue-600 dark:text-blue-400">Company Details Editor</h1>
        </div>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {companies.map((c, index) => (
                    <tr
                      ...
                      className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${selectedCompanyId === c.id ? 'bg-blue-50 dark:bg-blue-900/50 ring-2 ring-inset ring-blue-500 dark:ring-blue-600' : ''}`}
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400" onClick={() => handleSelectCompany(c.id)}>{index + 1}</td>
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100 cursor-pointer" onClick={() => handleSelectCompany(c.id)}>
